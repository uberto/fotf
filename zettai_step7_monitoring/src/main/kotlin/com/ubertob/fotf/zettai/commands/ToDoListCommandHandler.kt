...
class ToDoListCommandHandler<CTX>(
    ...
    override fun invoke(command: ToDoListCommand): ToDoListCommandOutcome =
        contextProvider.tryRun(
            when (command) {
                ...
                is UpdateToDoItem -> command.execute()
    ...
    private fun UpdateToDoItem.execute(): CommandOutcomeReader<CTX> =
        eventStore.retrieveByNaturalKey(UserListName(user, name))
            .transform { listState ->
                when (listState) {
                    is ActiveToDoList -> {
                        listState.items.firstOrNull() { it.description == oldItemName }
                            ?.let { ItemModified(listState.id, it, newItem).asCommandSuccess() }
                            ?: ToDoListCommandError("item $oldItemName not found!").asFailure()
                    }
    ...
    private fun DeleteToDoItem.execute(): CommandOutcomeReader<CTX> =
        eventStore.retrieveByNaturalKey(UserListName(user, name))
            .transform { listState ->
                when (listState) {
                    is ActiveToDoList -> {
                        listState.items.firstOrNull() { it.description == itemName }
                            ?.let { ItemRemoved(listState.id, it).asCommandSuccess() }
                            ?: ToDoListCommandError("item $itemName not found!").asFailure()
                    }
